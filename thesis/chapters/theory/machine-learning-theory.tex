...
Thomas et. al in \cite{thomas2020machine} add to these layers so-called rectified linear unit layer (\gls{relu}) and define its responsibility as "changing the negative pixel values in the image to zero, which gives us another stack of images with no negative values"~\cite{thomas2020machine}.
...
In the presented solution, the authors use a transfer learning technique wherein the previously trained model is used in a similar problem.
