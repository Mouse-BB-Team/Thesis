...
Solutions presented in previous chapters were deployed to the Heroku Cloud Application Platform\upperref{itm:heroku} to deliver the possibility of using the system among a wide range of users.
...
The deployment process is performed using the idea of containerization based on Docker\upperref{itm:docker} which is a pretty convenient way of application delivery to the web server.
...
It turned out that the database in such a plan permit to store only 20,000 rows overall which is unacceptable for the data stream with the time resolution measured in milliseconds because a single user may cause exceeding the limit in just a few minutes, which disqualifies the Heroku as a service provider for multi-user environment.
Moreover, the described cloud system suspends the working of the application when it is idle.
